...
F_B = D(:,3);                        % fluorescence of beacon samples
F_BT = D(:,4:11);                    % fluorescence of beacon and target samples
F_BT_mid = F_BT(lowtemp:hightemp,:); % fluorescence for temperature range

conc = readmatrix(file,'Sheet','conc'); % target concentration M
...
beta = p*alpha; % Estimate for one target bound
...
for m = 1:g % loop over temperatures
 ...
 % Initial upper and lower bounds; Set constraints and constraining regions as needed   
    ...
    ub = [0.01 0.01];     
    elseif (m>1) && (m<=15) 
        lb = [KBYY_opt(m-1) KBY_opt(m-1)];   
        ub = [1.2*KBYY_opt(m-1) 1.5*KBY_opt(m-1)];  
    else 
        lb = [KBYY_opt(m-1) KBY_opt(m-1)];   
        ub = [1.5*KBYY_opt(m-1) 1.5*KBY_opt(m-1)]; 
    end

        % Initial dissociation constants 
        K = [KBYY(m) KBY(m)]; % [Molar]  
        k3 = KYY(m);
   
        fun = @(K) obj_fluoro(K, Y, B,k3,alpha, beta, gamma, delta,conc, KB_mid, B_init, F_BT_mid,w,f,m);
...
for m = 1:g     % Loop over temperatures
    for j = 1:f % Loop over concentrations
        ...
        optfsolve = optimoptions('fsolve','Algorithm','levenberg-marquardt','Display','iter', ...
            'OptimalityTolerance',1e-6,'FunctionTolerance',1e-60,'StepTolerance',1e-60);

        x = fsolve(@(x)free(x,conc,k3,KBYY_opt,KBY_opt,B_init,m,j),x,optfsolve);
         ...
         Fluoro(m,j) = (alpha*Yfopt(m,j) + beta*KBYY_opt(m))/(Yfopt(m,j) +...
             KBYY_opt(m) + (1/Yfopt(m,j))*(KBYY_opt(m)*KBY_opt(m) +...
             KBYY_opt(m)*KBY_opt(m)*KB_mid(m))) +...
             (gamma+delta*KB_mid(m))/(1+KB_mid(m) +... 
             (Yfopt(m,j)/KBY_opt(m))*(1+(Yfopt(m,j)/KBYY_opt(m))));
...
for m = 1:g         % Loop over temperatures
    for j = 1:200   % Loop over fit concentrations
        ...
        Fluoro_fit(m,j) = (alpha*conc_fit(j) + beta* KBYY_opt(m))/(conc_fit(j) +...
             KBYY_opt(m) + (1/conc_fit(j))*(KBYY_opt(m)*KBY_opt(m) +...
             KBYY_opt(m)*KBY_opt(m)*KB_mid(m))) +...
             (gamma+delta*KB_mid(m))/(1+KB_mid(m) +... 
             (conc_fit(j)/KBY_opt(m))*(1+(conc_fit(j)/KBYY_opt(m))));
...
filename =  'TwoSite_Fit_KDBY_KDBYY.xlsx';
...
writematrix(KBYY_opt,filename,'Sheet','KD_opt','Range','C1');
writematrix(Res,filename,'Sheet','Residuals');
writematrix(Rsquare,filename,'Sheet','R2');

%% Plots of KDs, Bfopt, Yfopt, and fits
...
plot(Temperature,KBY_opt)
plot(Temperature,KBY)
title('Dissociation Constants')
xlabel('Temperature (Celsius)')
ylabel('Molar')
legend('KD_{BYY,opt}','KD_{BYY}','KD_{BY,opt}','KD_{BY}')
hold off
...
ylim([0,2])
title('Fluorescence fits')
xlabel('Temperature (Celsius)')
ylabel('Fluorescence')
subtitle(['Beta = ', num2str(beta),])
hold off

%% Fits at select temperatures
...
ylim([0,2])
subtitle(['T = ', num2str(Temperature(q)),'; R^2 =', num2str(Rsquare(q))]);
hold off
...
function SSR_sum = obj_fluoro(K,Y,B,k3,alpha, beta, gamma, delta,conc, KB_mid, B_init, F_BT_mid,w,f,m)
...
for j = 1:f
        ...
        Fluoro(m,j) = (alpha*Yf(m,j) + beta*K(1))/(Yf(m,j) +...
             K(1) + (1/Yf(m,j))*(K(1)*K(2) +...
             K(1)*K(2)*KB_mid(m))) +...
             (gamma+delta*KB_mid(m))/(1+KB_mid(m) +... 
             (Yf(m,j)/K(2))*(1+(Yf(m,j)/K(1))));
...
% Function to minimize (sum of the sums)
    SSR_sum = sum(SSR); 
     ...
     function h = model(x,conc,k3,K,B_init,j)
        ...
        h(1) = (2*(x(1)^2)/k3) + x(1) + (x(1)*x(2)/K(2)) + (2*x(2)*x(1)^2)/(K(2)*K(1))-conc(j);
        h(2) = x(2)*((x(1)^2)/(K(2)*K(1))+(x(1)/K(2))+1)-B_init;
...
function p = free(x,conc,k3,K1_opt,K2_opt,B_init,m,j)

% x(1) = free target; x(2) = free beacon;  

        p(1) = (2*(x(1)^2)/k3) + x(1) + ((x(1)*x(2))/K2_opt(m)) +...
            (2*x(2)*(x(1)^2))/(K2_opt(m)*K1_opt(m))-conc(j);
        p(2) = x(2)*((x(1)^2)/(K2_opt(m)*K1_opt(m))+(x(1)/K2_opt(m))+1)-B_init;
